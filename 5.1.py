...
# Convertions
final = -1
for seed in range (len(database[0])) :
    # Graine vers Sol (seed-to-soil)
    soil = database[0][seed]
    for line in range (len(database[1])) :
        if database[1][line][1] <= database[0][seed] <= database[1][line][1]+database[1][line][2]-1 :
            soil = database[0][seed] - database[1][line][1]
